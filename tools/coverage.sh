...
#
# libecrs ships as -betterC, which has no druntime, while D's -cov registers
...
# supplies a druntime `main` when `EcrsCoverage` is set.
#
...
# dub lists only libecrs' own modules; libfp and bc-threads have to be
# compiled in too, since -I alone would leave their symbols undefined at
# link time. The paths left absolute above are exactly the ones outside the
# project.
...
# ran.
"$compiler" -cov -g -unittest "${versionFlag}EcrsCoverage" \
	"${importPaths[@]/#/-I}" "${sources[@]}" -of="$out/ecrs-coverage$exeSuffix"
...
"./$out/ecrs-coverage$exeSuffix" --DRT-testmode=run-main \
...
# everything that is not libecrs' own -- libfp and bc-threads get measured
# by their own copies of this script.
cd "$out"
