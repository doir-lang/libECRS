...
///
/// NOTE: `@nogc nothrow:` colon-attributes don't propagate into a struct's
/// member functions in D, so every method below spells `@nogc nothrow` out
/// explicitly rather than relying on the module-level colon attribute.
module ecrs.storage;
...
///
/// Deliberately left plain-old-data (no disabled postblit, no destructor):
/// `Context` keeps these inside an `fp.dynarray`, which relocates elements
/// via raw `memcpy` on growth rather than via D copy/move semantics, so a
/// C++-style non-copyable/RAII discipline wouldn't actually hold here.
/// Ownership is managed explicitly instead - `Context.free()` is the only
/// code that should ever call `free()`.
struct ComponentStorage {
	...
	this(size_t elementSize, size_t reservedElementCount = defaultReservedElementCount, FinalizeFunction finalizeFunction = null) @trusted @nogc nothrow {
		this.elementSize = elementSize;
		this.finalizeFunction = finalizeFunction;
		fp.dynarray.reserve(raw, reservedElementCount * elementSize);
	}

	/// Runs this storage's finalizer (if any) on element `i`, then zero-fills
	/// its bytes. The zero-fill matters: it makes a later, redundant
	/// finalization of the same slot (e.g. `free()` revisiting a slot already
	/// finalized by `remove()`/`Context.removeComponent`) a safe no-op rather
	/// than a double-free, since a zeroed `Relation.related` reads back null.
	void finalizeElement(size_t i) @trusted @nogc nothrow {
		if (finalizeFunction is null) return;
		void* p = get(i);
		finalizeFunction(p);
	...
	void free() @trusted @nogc nothrow {
		if (raw !is null) {
			if (finalizeFunction !is null)
				foreach (i; 0 .. size())
					finalizeElement(i);
			fp.dynarray.free(raw);
		}
		elementSize = invalid;
	}

	size_t size() const @trusted @nogc nothrow {
		return raw is null || elementSize == invalid || elementSize == 0 ? 0 : fp.dynarray.length(cast(ubyte*) raw) / elementSize;
	}
	bool empty() const @nogc nothrow { return size() == 0; }

	inout(T)* data(T)() inout @trusted @nogc nothrow {
		assert(T.sizeof == elementSize);
		return cast(inout(T)*) raw;
	}

	void* get(size_t i) @trusted @nogc nothrow { assert(i < size()); return raw + i * elementSize; }
	const(void)* get(size_t i) const @trusted @nogc nothrow { assert(i < size()); return raw + i * elementSize; }
	ref inout(T) get(T)(size_t i) inout @trusted @nogc nothrow { assert(i < size()); return data!T()[i]; }

	/// Grows storage by `count` zero-filled, type-erased elements.
	void allocate(size_t count = 1) @trusted @nogc nothrow {
		immutable oldBytes = raw is null ? 0 : fp.dynarray.length(raw);
		fp.dynarray.grow(raw, elementSize * count);
		memset(raw + oldBytes, 0, elementSize * count);
	}
	/// Grows storage by `count` elements, each set to `T.init` (D's equivalent
	/// of the C++ version's placement-new default construction).
	void allocate(T)(size_t count = 1) @trusted @nogc nothrow {
		assert(T.sizeof == elementSize);
		immutable oldCount = size();
		fp.dynarray.grow(raw, elementSize * count);
		T* d = data!T();
		foreach (i; 0 .. count)
			d[oldCount + i] = T.init;
	...
	void* getOrAllocate(size_t e) @trusted @nogc nothrow {
		immutable sz = size();
		if (sz <= e) allocate(e - sz + 1);
		return get(e);
	}
	ref T getOrAllocate(T)(size_t e) @trusted @nogc nothrow {
		immutable sz = size();
		if (sz <= e) allocate!T(e - sz + 1);
		return get!T(e);
	}

	private void swapBytes(size_t a, size_t b) @trusted @nogc nothrow {
		assert(a < size());
		assert(b < size());
		fp.dynarray.swapRange!ubyte(raw, a * elementSize, b * elementSize, elementSize);
	}
	...
	bool swap(size_t componentIdVal, ref EntityComponentIndices entityComponentIndices, size_t a, size_t b, bool swapIfOneElementless = false) @trusted @nogc nothrow {
	...
	template swap(Tcomponent, size_t Unique = 0) {
		bool swap(ref EntityComponentIndices entityComponentIndices, size_t a, size_t b, bool swapIfOneElementless = false) @nogc nothrow {
			return this.swap(componentId!(Tcomponent, Unique)(), entityComponentIndices, a, b, swapIfOneElementless);
		}
	}
	...
	bool remove(size_t componentIdVal, ref EntityComponentIndices entityComponentIndices, EntityId e) @trusted @nogc nothrow {
		immutable sz = size();
		...
		swapBytes(indices[componentIdVal], sz - 1);
		...
		finalizeElement(sz - 1);
		fp.dynarray.popBackCount(raw, elementSize);
		indices[componentIdVal] = invalid;
		return true;
	}
	template remove(Tcomponent, size_t Unique = 0) {
		bool remove(ref EntityComponentIndices entityComponentIndices, EntityId e) @nogc nothrow {
			return this.remove(componentId!(Tcomponent, Unique)(), entityComponentIndices, e);
		}
	}

	/// Applies a permutation: `order[i]` is the new position of the element
	/// currently at index `i`.
	void reorder(size_t componentIdVal, ref EntityComponentIndices entityComponentIndices, const(size_t)[] order) @trusted @nogc nothrow {
		immutable n = size();
		...
		foreach (i; 0 .. n)
			while (swaps[i] != i) {
				this.swap(componentIdVal, entityComponentIndices, swaps[i], i, false);
				immutable t = swaps[swaps[i]]; swaps[swaps[i]] = swaps[i]; swaps[i] = t;
			}
	}
	template reorder(Tcomponent, size_t Unique = 0) {
		void reorder(ref EntityComponentIndices entityComponentIndices, const(size_t)[] order) @nogc nothrow {
			this.reorder(componentId!(Tcomponent, Unique)(), entityComponentIndices, order);
		}
	}

	/// Sorts by `less(elementPtr(a), elementPtr(b))`, ignoring entity identity.
	void sort(size_t componentIdVal, ref EntityComponentIndices entityComponentIndices, LessFunction less) @trusted @nogc nothrow {
		...
		bool cmp(size_t a, size_t b) @trusted @nogc nothrow { return less(get(a), get(b)); }
		stdSort!cmp(order[0 .. n]);
		this.reorder(componentIdVal, entityComponentIndices, order[0 .. n]);
	}
	/// Sorts by `less(elementPtr(a), entityOf(a), elementPtr(b), entityOf(b))`.
	void sort(size_t componentIdVal, ref EntityComponentIndices entityComponentIndices, LessWithEntityFunction less) @trusted @nogc nothrow {
		import std.algorithm.sorting : stdSort = sort;
		...
		bool cmp(size_t a, size_t b) @trusted @nogc nothrow { return less(get(a), entities[a], get(b), entities[b]); }
		stdSort!cmp(order[0 .. n]);
		this.reorder(componentIdVal, entityComponentIndices, order[0 .. n]);
	}
	template sortByValue(Tcomponent, size_t Unique = 0) {
		void sortByValue(ref EntityComponentIndices entityComponentIndices) @nogc nothrow {
			this.sort(componentId!(Tcomponent, Unique)(), entityComponentIndices, &sortByValueLess!Tcomponent);
		}
	}
	void sortMonotonic(size_t componentIdVal, ref EntityComponentIndices entityComponentIndices) @nogc nothrow {
		this.sort(componentIdVal, entityComponentIndices, &monotonicLess);
	}
	template sortMonotonic(Tcomponent, size_t Unique = 0) {
		void sortMonotonic(ref EntityComponentIndices entityComponentIndices) @nogc nothrow {
			this.sort(componentId!(Tcomponent, Unique)(), entityComponentIndices, &.monotonicLess);
		}
	}
}
...
unittest {
	...
	ComponentStorage storage = ComponentStorage(Vec2.sizeof);
	scope(exit) storage.free();
...
unittest {
	...
	ComponentStorage storage = ComponentStorage(int.sizeof);
	scope(exit) storage.free();
...
unittest {
	...
	ComponentStorage storage = ComponentStorage(double.sizeof);
	scope(exit) storage.free();
	...
	assert(constStorage.get(0) !is null);
}
...
unittest {
	...
	ComponentStorage storage = ComponentStorage(short.sizeof);
	scope(exit) storage.free();
